...
Como sabemos, el valor  $t= `r summary(reg)$coefficients[1,3]`$ es `r tex<-"menor";if(summary(reg)$coefficients[2,3]>qt(0.975 , df =nrow(data)-2)) tex<-"menor"; tex` que 
$t_{`r (nrow(data)-2)`}(\frac{\alpha}{2})= `r qt(0.975 , df =nrow(data)-2)`$
`r tex<-"se rechazo"; if(summary(reg)$coefficients[2,3]>qt(0.975 , df =nrow(data)-2)) tex<-"no rechazo"; tex`
$H_0: \beta_2=0$.
